...
VIDEO_PATH = r"D:\Documentos\MEGA\Desenvolvimento\VS Code Workspace\FIAP Pós - IA\Tech Challenge - 4ª Fase\video-tc4.mp4"

# Define o intervalo de frames a serem analisados para melhorar a performance
FRAMES_INTERVAL_TO_ANALYZE = 30
...
def detect_activities(frame):
    ...
    if pose_results.pose_landmarks:
        ...
        if pulso_esq.y < olho_esq.y < ombro_esq.y or pulso_dir.y < olho_dir.y < ombro_dir.y:
            results.append("Braços levantados")
...
def analyze_video(video_path):
    ...
    while True:
        ...
        if total_frames % FRAMES_INTERVAL_TO_ANALYZE == 0:
            ...
            # Desenha um retângulo em cada rosto detectado no frame
            ...
            print(f" Frame: {total_frames}, Atividades: {activities_detected}, Emoções: {emotions}, Rostos Detectados: {len(faces_coords)}")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(VIDEO_PATH):
        raise Exception(f"Nenhum arquivo foi encontrado no caminho informado. Verifique a informação e tente novamente. ({VIDEO_PATH})")
